...
public static class DialogPhaser
{
    ...
    public static void PhaseNode(Node<Dialog> node, string text)
    {
        ...
        else if (dialogInfo.Split('：').Length == 0) // 功能性节点
        {
            ...
            Debugger.LogFail("Unsupport content symbol \':\', please replace it.");
        ...
        if (nodeInfo != "")
        {
            ...
            for (int i = 0; i < parameters.Length; i++)
            {
                ...
                if (semantic == "ccomplete")
                {
                    ...
                    node.nodeEntity.completeConditons = value.Trim().RemoveBrackets().Split('&').ToList();
                ...
                if (semantic == "cdivider")
                {
                    ...
                    node.nodeEntity.dividerConditions = cparams[0].Trim().RemoveBrackets().Split('&').ToList();
